...
description: AionUi setup guide — free, open-source desktop office agent supporting Gemini CLI, Claude Code, Codex, and more. Connect to TokenFactory for multi-model AI workflows.
---
...
## TokenFactory Configuration Steps

1. **Copy API key in TokenFactory**
...
3. **Add New Provider**
   - Click "Add Model"
   - Select TokenFactory
   ![Add TokenFactory Provider](/assets/aionui/tokenfactory_provider.png)

4. **Configure API Information**
   - API Address: Fill in your TokenFactory site address (format: `https://<your-tokenfactory-domain>/v1`)
   - API Key: Paste the API Key copied from TokenFactory console

5. **Add Models**
   - Select the model to add from the dropdown
   - Model name should match the model name configured in TokenFactory
   - Select the appropriate request protocol

6. **Start Using**
   - Return to the chat page
   - Select the configured TokenFactory model to start a conversation
