...
description: FluentRead configuration guide — open-source browser translation plugin with 20+ engines and AI model support. Connect to TokenFactory for immersive bilingual reading.
---

<Callout type="info" title="Chat Settings Options">
  In the System Settings -> Chat Settings of the TokenFactory console, you can add
...
### Import Configuration from TokenFactory Console (Recommended)

After the FluentRead plugin is installed in your browser, opening the TokenFactory console -> Token Management page will display a prompt to add FluentRead.
...
After confirming the import, the TokenFactory configuration in FluentRead will be enabled.
...
| ------------------- | --------------------------------------- |
| Translation Service | TokenFactory                                  |
| Access Token        | TokenFactory Key                              |
| TokenFactory Endpoint     | TokenFactory Deployment Address (without /v1) |
