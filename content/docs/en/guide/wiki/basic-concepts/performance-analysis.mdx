...
TokenFactory provides two types of performance analysis capabilities:
...
environment:
  ...
  - PYROSCOPE_APP_NAME=token-factory
...
export PYROSCOPE_APP_NAME=token-factory
...
```bash
# Check env vars
docker exec token-factory env | grep -E "PPROF|PYROSCOPE"
```
