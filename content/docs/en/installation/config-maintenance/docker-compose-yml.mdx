...
This document details the Docker Compose configuration options for TokenFactory, applicable to various deployment scenarios.

## Basic Configuration Structure

The Docker Compose configuration file `docker-compose.yml` defines how the TokenFactory service and its dependent services (such as MySQL, Redis) are deployed.
...
services:
  token-factory:
    image: calciumion/token-factory:latest
    ...
    volumes:
      ...
      - ./logs:/app/logs
    environment:
      - SQL_DSN=postgresql://root:123456@postgres:5432/token-factory # ⚠️ IMPORTANT: Change the password in production!
      #      - SQL_DSN=root:123456@tcp(mysql:3306)/token-factory  # Point to the mysql service, uncomment if using MySQL
...
For testing purposes, you can use the following simplified version, which only includes the TokenFactory service itself:
...
### TokenFactory Service Configuration

| Parameter        | Description                                                                    |
| ---------------- | ------------------------------------------------------------------------------ |
| `image`          | Image name, usually `calciumion/token-factory:latest` to get the latest version      |
...
| `environment`    | Environment variable settings, used to configure TokenFactory behavior              |
...
TokenFactory supports various environment variable configurations. Here are some commonly used ones:

| Environment Variable | Description                                         | Example                               |
| -------------------- | --------------------------------------------------- | ------------------------------------- |
| `SQL_DSN`            | Database connection string                          | `root:123456@tcp(mysql:3306)/token-factory` |
...
```yaml
services:
  token-factory-master:
    image: calciumion/token-factory:latest
    container_name: token-factory-master
    restart: always
    ports:
      - '3000:3000'
    environment:
      - SQL_DSN=root:123456@tcp(mysql:3306)/token-factory
      - REDIS_CONN_STRING=redis://redis
...
services:
  token-factory-slave:
    image: calciumion/token-factory:latest
    container_name: token-factory-slave
    restart: always
    ports:
