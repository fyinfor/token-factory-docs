...
This document provides methods and best practices for updating the TokenFactory system to ensure a smooth upgrade to the latest version.
...
2.  **Check update logs**: Review the update content for the latest version on [GitHub Releases](https://github.com/Calcium-Ion/token-factory/releases)
...
If you have deployed TokenFactory using a single Docker container, you can update by following these steps:
...
# 停止并移除旧容器
docker stop token-factory
docker rm token-factory
...
2.  Find the TokenFactory application and click the **Upgrade** button
...
2.  Find the TokenFactory container, click **More** -> **Recreate**
...
If you have deployed TokenFactory by compiling from source code, the update steps are as follows:
...
```shell
# 拉取特定版本的镜像
docker pull calciumion/token-factory:v1.x.x
...
docker run --name token-factory -d --restart always \
  ...
  -e TZ=Asia/Shanghai \
  -v /your/data/path:/data \
  calciumion/token-factory:v1.x.x
```
