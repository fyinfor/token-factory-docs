...
description: AionUi セットアップガイド — Gemini CLI、Claude Code、Codex などに対応した無料オープンソースのデスクトップオフィスAgent。TokenFactory 接続対応。
---
...
3. **新しいプロバイダーを追加**
   - 「モデルを追加」をクリック
   - TokenFactoryを選択
   ![TokenFactoryプロバイダーを追加](/assets/aionui/tokenfactory_provider.png)

4. **API情報を設定**
   - APIアドレス：TokenFactoryサイトアドレスを入力（形式：`https://<your-tokenfactory-domain>/v1`）
   - APIキー：TokenFactoryコンソールからコピーしたAPIキーを貼り付け

5. **モデルを追加**
   - ドロップダウンから追加するモデルを選択
   - モデル名はTokenFactoryで設定したモデル名と一致する必要があります
   - 適切なリクエストプロトコルを選択

6. **使用開始**
   - チャットページに戻る
   - 設定したTokenFactoryモデルを選択して会話を開始
