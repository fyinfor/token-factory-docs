...
このドキュメントでは、Token Factoryがサポートするすべての環境変数とその設定について説明します。これらの環境変数を設定することで、システムの動作をカスタマイズできます。

<Callout type="info" title="ヒント">
  TokenFactory は `.env` ファイルから環境変数を読み込むことをサポートしています。`.env.example`
...
| 環境変数                | 説明                           | デフォルト値                   | 例                                                                                                                             |
| ----------------------- | ------------------------------ | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| `SQL_DSN`               | データベース接続文字列         | SQLite (`one-api.db`) | MySQL: `SQL_DSN=root:123456@tcp(localhost:3306)/token-factory` \| PostgreSQL: `SQL_DSN=postgresql://root:123456@postgres:5432/token-factory` |
...
| `SQLITE_PATH`           | SQLiteデータベースパス         | `one-api.db`             | `SQLITE_PATH=/var/lib/token-factory/token-factory.db`                                                                                        |
...
| `FRONTEND_BASE_URL`         | フロントエンドベースURL（スレーブノードで使用、未一致のルーティングをこのアドレスにリダイレクト） | -        | `FRONTEND_BASE_URL=https://<your-tokenfactory-domain>` |
...
| `PYROSCOPE_APP_NAME` | レポートするアプリケーション名     | `token-factory`    | `PYROSCOPE_APP_NAME=my-api`  |
...
| `HOSTNAME` | レポートするホスト名ラベル           | `token-factory`    | `HOSTNAME=node-1`            |
