...
3. **インストール** をクリックします
4. 以下の基本オプションを設定します：
   - **コンテナ名**：カスタマイズ可能、デフォルトは `token-factory`
...
1. 宝塔パネルでウェブサイトディレクトリを作成します（例：`/www/wwwroot/token-factory`）
...
3. イメージ名を入力：`calciumion/token-factory:latest`
4. **送信** をクリックし、イメージのプルが完了するまで待ちます
5. プル完了後、**コンテナリスト** に入り、**コンテナ作成** をクリックします
6. 以下の情報を入力します：
   - **コンテナ名**：`token-factory`（カスタマイズ可能）
   - **イメージ**：プルした `calciumion/token-factory:latest` を選択
