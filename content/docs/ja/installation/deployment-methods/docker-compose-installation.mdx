...
このドキュメントでは、Docker Compose を使用して TokenFactory をデプロイする詳細な手順を説明します。
...
1. TokenFactory デプロイ用のディレクトリを作成します。
...
- **指定サービスのログ**（例：`token-factory`、`mysql`、`redis`）
...
- **コンテナ名を使用してログを表示**（`container_name` が設定されている場合、例：設定内の `token-factory`）
