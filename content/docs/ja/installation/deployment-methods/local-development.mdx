...
このドキュメントは、TokenFactory プロジェクトをローカル環境でセットアップし開発するための詳細な手順を提供します。プロジェクト開発に参加したい、またはカスタム開発を行いたい開発者向けです。
...
まず、GitHub から TokenFactory リポジトリをローカルにクローンします。
...
TokenFactory は `.env` ファイルを介した環境変数の設定をサポートしています。`.env` ファイルを作成します（`.env.example` からコピーできます）。
...
```bash
PORT=3000
SQL_DSN=root:password@tcp(localhost:3306)/token-factory   # MySQL を使用する場合、コメント解除して変更
...
TokenFactory のフロントエンドコードは `web` ディレクトリにあり、React と [semi design コンポーネントライブラリ](https://semi.design/zh-CN) を使用して開発されています。
...
TokenFactory プロジェクトのディレクトリ構造：

```
token-factory/                                 # プロジェクトルートディレクトリ
...
1.  [GitHub Issues](https://github.com/fyinfor/token-factory/issues) を確認する
