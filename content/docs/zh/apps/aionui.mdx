...
description: AionUi 教程 — 免费开源桌面办公 Agent，支持 Gemini CLI、Claude Code、Codex 等多种 AI 代理。对接 TokenFactory 实现多模型工作流。
---
...
提供商类型：TokenFactory 支持的类型  
API 密钥：于 TokenFactory 获取  
API 地址：TokenFactory 站点地址（例如：`https://<your-tokenfactory-domain>/v1`）

### 配置步骤

1. **在 TokenFactory 中复制 API key**
...
3. **添加新的提供商**
   - 点击"添加模型"
   - 选择 TokenFactory
   ![添加 TokenFactory 提供商](/assets/aionui/tokenfactory_provider.png)

4. **配置 API 信息**
   - API 地址：填写您的 TokenFactory 站点地址（格式：`https://<your-tokenfactory-domain>/v1`）
   - API 密钥：粘贴从 TokenFactory 控制台复制的 API Key

5. **添加模型**
   - 下拉选择需要添加的模型
   - 模型名称应与 TokenFactory 中配置的模型名称一致
   - 选择合适的请求协议

6. **开始使用**
   - 返回聊天页面
   - 选择已配置的 TokenFactory 模型开始对话
