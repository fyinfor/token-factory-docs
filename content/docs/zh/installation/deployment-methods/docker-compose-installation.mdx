...
本文档提供了使用Docker Compose部署Token Factory的详细步骤。
...
- **指定服务日志**（示例：`token-factory`、`mysql`、`redis`）
...
- **使用容器名查看日志**（当 `container_name` 已设置，如配置里的 `token-factory`）
